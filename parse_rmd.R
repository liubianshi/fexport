...
default_meta <- list(
    infile            = args$infile,
    outfile           = sub("\\.[Rr](md|markdown)$", ".knit.md", args$infile),
...
meta <- yaml::read_yaml(args$yaml_file)[[args$outformat]] |>
        complete_config_info(default_meta)

diy_output_format <- rlang::exec(rlang::parse_expr(meta$out), !!!(meta$opt))
...
if (is.null(args$result_file)) {
    args$result_file <- file.path(meta$intermediates_dir, "rmd_meta.yaml")
}
